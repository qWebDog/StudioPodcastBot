...
from keyboards import (
    welcome_kb, dates_kb, time_slots_kb, services_kb, confirm_kb, 
    format_date_display, saved_data_kb
)
...
async def select_date(cb: CallbackQuery, state: FSMContext):
    ...
    if not slots:
        await cb.message.answer("❌ На эту дату все часы заняты. Выберите другую."); await cb.answer(); return
    ...
    await cb.message.answer(
        ...
        reply_markup=time_slots_kb(slots, []), parse_mode="Markdown"
    )
    await cb.answer()
...
async def finish_slots(cb: CallbackQuery, state: FSMContext):
    ...
    if user and user.client_name and user.phone:
        await cb.message.answer(
            ...
            f"👤 Имя: `{user.client_name}`\n📞 Телефон: `{user.phone}`",
            reply_markup=saved_data_kb(), parse_mode="Markdown"
        )
    else:
...
async def save_name(m: Message, state: FSMContext):
    ...
    await m.answer(f"📞 **Шаг 4/5:** Введите номер телефона:")
...
async def _go_to_services(event, state: FSMContext):
    ...
    await event.bot.send_message(
        event.chat.id, 
        "🛠 **Шаг 5/5:** Выберите доп. услуги (или 'Завершить'):", 
        reply_markup=services_kb(svcs), parse_mode="Markdown"
    )
...
# (my_bookings, my_cancel, _notify_admins остаются без изменений из прошлой версии)
