...
STUDIO_TZ = ZoneInfo("Europe/Moscow")  # 🌍 Измените при необходимости
...
# 🔄 Единый переключатель меню (одно сообщение)
...
async def go_price(cb: CallbackQuery): await switch_view(cb, "price"); await cb.answer()
...
async def select_month(cb: CallbackQuery, state: FSMContext):
    ym = cb.data.split(":")[1]; await state.update_data(year_month=ym)
    ...
    if not dates:
        ...
        await cb.message.answer("❌ В этом месяце нет свободных дней.", reply_markup=kb.as_markup()); await cb.answer(); return
...
async def select_date(cb: CallbackQuery, state: FSMContext):
    ...
    if not slots:
        ...
        await cb.message.answer("❌ Нет доступных часов для бронирования.", reply_markup=kb.as_markup()); await cb.answer(); return
    ...
    kb.row(InlineKeyboardButton(text="⬅️ Назад к датам", callback_data="back_to_date"), InlineKeyboardButton(text="❌ Отмена", callback_data="book_cancel"))
    await cb.message.answer(f"📆 {format_date_display(date_iso)} | 💰 **{price_per_hour}₽/час**\n⏰ **Шаг 3/6:** Выберите нужные часы:", reply_markup=kb.as_markup(), parse_mode="Markdown")
    await cb.answer()
...
async def finish_slots(cb: CallbackQuery, state: FSMContext):
    ...
    if user and user.client_name and user.phone:
        ...
        ).row(InlineKeyboardButton(text="⬅️ Назад к времени", callback_data="back_to_slots"), InlineKeyboardButton(text="❌ Отмена", callback_data="book_cancel"))
        await cb.message.answer(f"👤 **Шаг 4/6:** Данные уже сохранены:\n👤 Имя: `{user.client_name}`\n📞 Телефон: `{user.phone}`", reply_markup=kb.as_markup(), parse_mode="Markdown")
    else:
        await state.set_state(BookFSM.name)
        hint = f"\n(Ранее было: `{user.client_name}`)" if user and user.client_name else ""
        kb = InlineKeyboardBuilder().row(InlineKeyboardButton(text="⬅️ Назад к времени", callback_data="back_to_slots"), InlineKeyboardButton(text="❌ Отмена", callback_data="book_cancel"))
...
async def save_name(m: Message, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder().row(InlineKeyboardButton(text="⬅️ Назад к имени", callback_data="back_to_name"), InlineKeyboardButton(text="❌ Отмена", callback_data="book_cancel"))
...
async def back_to_slots(cb: CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data(); sel = data.get("selected_slots", [])
    async with async_session() as s:
        res = await s.execute(select(Slot).where(Slot.date == data["date"], Slot.is_active, ~Slot.is_booked).order_by(Slot.start_time))
        slots = res.scalars().all()
    kb = InlineKeyboardBuilder()
    ...
    await cb.message.edit_text(f"📆 {format_date_display(data['date'])} | 💰 **{price}₽/час**\n⏰ Выберите часы:", reply_markup=kb.as_markup(), parse_mode="Markdown")
    await cb.answer()
...
async def _show_services(event, state: FSMContext):
    await state.set_state(BookFSM.services); await state.update_data(selected_services=[])
    async with async_session() as s:
    ...
    kb.button(text="✅ Завершить выбор", callback_data="book_svcs_done"); kb.adjust(1)
    kb.row(InlineKeyboardButton(text="⬅️ Назад к телефону", callback_data="back_to_phone"), InlineKeyboardButton(text="❌ Отмена", callback_data="book_cancel"))
...
async def manage_services(cb: CallbackQuery, state: FSMContext):
    ...
    if cb.data == "book_svcs_done":
        svc_total = sum(s["price"] for s in data.get("selected_services", [])); slot_total = 0; times_str = []
        async with async_session() as s:
        ...
        kb = InlineKeyboardBuilder().button(text="✅ Подтвердить бронь", callback_data="book_confirm")
        kb.row(InlineKeyboardButton(text="⬅️ Изменить услуги", callback_data="back_to_services"), InlineKeyboardButton(text="❌ Отмена", callback_data="book_cancel"))
        await cb.message.answer(f"📋 **Итог:**\n👤 {data['client_name']}\n📞 {data['phone']}\n📅 {format_date_display(data['date'])}\n⏰ {', '.join(times_str)}\n🎙️ Часы: {int(slot_total)}₽\n💰 Услуги: {int(svc_total)}₽\n💵 **Всего: {int(total)}₽**", reply_markup=kb.as_markup(), parse_mode="Markdown")
...
async def confirm_booking(cb: CallbackQuery, state: FSMContext):
    ...
    async with async_session() as s:
        ...
        for sid in data["selected_slots"]:
            sl = await s.get(Slot, sid)
            if not sl or sl.is_booked or not sl.is_active: await cb.message.answer(f"❌ Слот занят."); await state.clear(); return
            slots.append(sl)
        for sl in slots: sl.is_booked = True
        svc_total = sum(x["price"] for x in data.get("selected_services", [])); slot_total = sum(sl.price for sl in slots); total_price = slot_total + svc_total
        times_str = [f"{sl.start_time}-{sl.end_time}" for sl in slots]
        new_booking = Booking(user_tg_id=cb.from_user.id, slot_ids=json.dumps(data["selected_slots"]), services=json.dumps([x["id"] for x in data.get("selected_services", [])]), total_price=total_price)
        s.add(new_booking); await s.commit()
...
async def back_to_phone(cb: CallbackQuery, state: FSMContext): await state.set_state(BookFSM.phone); await save_phone(cb.message, state); await cb.answer() # Упрощено для стабильности
@router.callback_query(F.data == "back_to_services")
async def back_to_services(cb: CallbackQuery, state: FSMContext): await _show_services(cb.message, state); await cb.answer()
...
async def handle_reminder(cb: CallbackQuery):
    action, bid_str = cb.data.split(":"); bid = int(bid_str)
    async with async_session() as s:
        b = await s.get(Booking, bid)
        ...
        if action == "rem_confirm": b.status = "confirmed_reminder"; txt = "✅ Запись подтверждена!"
        else: b.status = "cancelled"; txt = "❌ Запись отменена."
            for sid in json.loads(b.slot_ids):
                sl = await s.get(Slot, sid)
                if sl: sl.is_booked = False
        await s.commit()
    try: await cb.message.edit_text(f"{txt}\n🆔 Бронь #{bid}")
    except: pass
