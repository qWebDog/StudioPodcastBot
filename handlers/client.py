    ...
    async with async_session() as s:
        ...
        for sid in data["selected_slots"]:
            ...
            if not sl or sl.is_booked or not sl.is_active:
                await cb.message.answer(f"❌ Слот {sl.start_time if sl else 'N/A'}-{sl.end_time if sl else ''} только что забронировали. Выберите другое время.")
                await state.clear(); return
            slots.append(sl)
...
async def show_my_bookings(cb: CallbackQuery):
    ...
    for b in bookings:
        ...
        msg += f"\n{st_icon} #{b.id} | {format_date_display(date_str)} ⏰ {times}\n💰 {int(b.total_price)}₽"
...
async def my_cancel(cb: CallbackQuery):
    ...
    await cb.message.edit_text("❌ Вы отменили запись. Слоты освобождены.")
    await cb.answer()
